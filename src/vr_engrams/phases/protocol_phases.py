        ...
        for condition in conditions:
            for _ in range(reps):
                ...
                if condition == "sound_a":
                    trial["frequency_hz"] = float(self.config.get("sound_a_frequency_hz", 8000.0))
                elif condition == "sound_b":
                    trial["frequency_hz"] = float(self.config.get("sound_b_frequency_hz", 12000.0))
...
class PreConditioningScenePhase(ExperimentPhase):
    ...
    def _run_scene_with_dropout(self, label: str, block_idx: int, scene_key: str, duration_sec: float) -> None:
        ...
        dropout_interval_sec = 10.0
        dropout_range = [2.0, 4.0]
        self.context.logger.log_event(
        ...
        while elapsed < duration_sec:
            ...
            drop_modality = self.context.rng.choice(["screen", "sound", "whisker"])
    ...
    def _deliver_scene_chunk(self, scene_id: str, duration_sec: float, dropped: str | None) -> None:
        if duration_sec <= 0:
            return
        if dropped != "screen":
            self.context.stimuli.deliver_visual(channel=f"screen_{scene_id.lower()}", duration_sec=min(0.001, duration_sec))
        if dropped != "sound":
            freq = 8000.0 if scene_id.upper() == "A" else 12000.0
            self.context.stimuli.deliver_sound(frequency_hz=freq, duration_sec=duration_sec, side="both")
        else:
            self._sleep(duration_sec)
        if dropped != "whisker":
            self.context.stimuli.deliver_puff(channel="puff", duration_sec=min(0.005, duration_sec))
